...
#  endif
   ...
   if ( d_Is_Son_Array_S     != NULL ) {  CUDA_CHECK_ERROR(  cudaFree( d_Is_Son_Array_S     )  );  d_Is_Son_Array_S     = NULL; }
#  if ( FLU_SCHEME == MHM  ||  FLU_SCHEME == MHM_RP  ||  FLU_SCHEME == CTU )
...
#     endif
      ...
      if ( h_Is_Son_Array_S [t] != NULL ) {  CUDA_CHECK_ERROR(  cudaFreeHost( h_Is_Son_Array_S [t] )  );  h_Is_Son_Array_S [t] = NULL; }
   } // for (int t=0; t<2; t++)
