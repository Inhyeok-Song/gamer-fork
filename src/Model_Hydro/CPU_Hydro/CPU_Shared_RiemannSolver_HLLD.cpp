...
#  if ( EOS == EOS_NUCLEAR )
#  ifdef MHD
   real Emag = (real)0.5*( SQR(Bx) + SQR(ByL) + SQR(BzL) );
#  else
   real Emag = NULL_REAL;
#  endif
   a2 = Hydro_Con2Cs2( Con_L[0], Con_L[1], Con_L[2], Con_L[3], Con_L[4], Con_L+NCOMP_FLUID,
                       PassiveFloor, Emag, EoS_General, EoS_AuxArray_Flt, EoS_AuxArray_Int, EoS_Table );
   if ( a2 != a2 )
#  endif
...
#  if ( EOS == EOS_NUCLEAR )
#  ifdef MHD
   Emag = (real)0.5*( SQR(Bx) + SQR(ByR) + SQR(BzR) );
#  else
   Emag = NULL_REAL;
#  endif
   a2 = Hydro_Con2Cs2( Con_R[0], Con_R[1], Con_R[2], Con_R[3], Con_R[4], Con_R+NCOMP_FLUID, PassiveFloor, Emag,
