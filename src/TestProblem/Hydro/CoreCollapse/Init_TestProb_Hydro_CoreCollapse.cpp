...
extern void nuc_eos_C_short( const real xrho, real *xenr, const real xye,
                             ...
                             const real *logtemp_mode, const real *entr_mode, const real *logeps_mode,
                ...
                const int lv, double AuxArray[] )
{
...
#endif
   nuc_eos_C_short( (dens*UNIT_D), &xenr, ye, &xtmp, &xent, &Useless, &Useless, &Useless,
   ...
   if ( Err != 0 )
   {
     printf("debug: EoS error is not zero %d\n", Err);
   }
   ...
   fluid[ENPY] = xent*dens;  // entropy [kB/baryon * dens]
   //fluid[ENGY] = pres / ( GAMMA - 1.0 ) + 0.5*( SQR(fluid[MOMX]) + SQR(fluid[MOMY]) + SQR(fluid[MOMZ]) ) / fluid[DENS];
   fluid[ENGY] = (dens/(UNIT_V*UNIT_V))*(xenr + EnergyShift) + 0.5*( SQR(fluid[MOMX]) + SQR(fluid[MOMY]) + SQR(fluid[MOMZ]) ) / fluid[DENS];
    ...
    if ( !EoS.EOS_POSTBOUNCE ) {
        ...
        for (int PID=0; PID<amr->NPatchComma[lv][1]; PID++)
        {
            ...
            for (int i=0; i<PS1; i++)  {  const double x = amr->patch[0][lv][PID]->EdgeL[0] + (i+0.5)*amr->dh[lv];
                ...
                entr = amr->patch[ amr->FluSg[lv] ][lv][PID]->fluid[ENPY][k][j][i];
        ...
        if (max_dens > bounceDens && max_entr > shockEntr)
        {
            if (MPI_Rank ==0) 
            {
                ...
                EoS.EOS_POSTBOUNCE = true;
                EoS.EOS_BOUNCETIME = Time[0];
            }
        }
    }
    // =====================================================================================
